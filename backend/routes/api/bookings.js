...
router.put('/:bookingId', requireAuth, async (req, res, next) => {
    ...
    allBookings.forEach(booking => {
        if (startDate === booking.startDate && endDate === booking.endDate) return res.status(403).json({"message": "Sorry, this spot is already booked for the specified dates", "statusCode": 403});

        if (startDate === booking.startDate || startDate >= booking.startDate && startDate <= booking.endDate) return res.status(403).json({"message": "Start date conflicts with an existing booking", "statusCode": 403});

        if (endDate === booking.endDate || endDate <= booking.endDate && endDate >= booking.startDate) return res.status(403).json({"message": "End date conflicts with an existing booking", "statusCode": 403});
    });
